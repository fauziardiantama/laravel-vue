<template>
          ...
          <CCard class="mb-4">
            ...
            <CCardBody v-if="magang_diterima && jawaban_diterima && proposal_diterima">
                ...
                >
                    ...
                    <template v-slot:none="data">
                        <CButton color="primary" v-if="data.value.status != 1" @click="openEditInstansiModal(data.value)">Edit</CButton>
                        <CButton color="danger" v-if="data.value.status != 1" @click="deleteBimbinganInstansi(data.value)">Delete</CButton>
                    </template>
                </table-lite>   
            </CCardBody>
            <CCardBody v-else>
            ...
            <CCardFooter  v-if="magang_diterima && jawaban_diterima && proposal_diterima">
                <CButton color="primary" @click="showTambahInstansiModal = true">Tambah</CButton>
            </CCardFooter>
            <CCardFooter v-else>
            </CCardFooter>
          </CCard>
    ...
    <CModal size="lg" backdrop="static" :visible="showEditInstansiModal" @close="closeModal">
        <CModalHeader>
            <CModalTitle>Edit Bimbingan Instansi</CModalTitle>
        </CModalHeader>
        <CModalBody>
            <CRow>
                <CCol>
                    <CFormInput type="date" id="tanggal_instansi_edit" placeholder="Tanggal"
                        label="Tanggal"
                        v-model="activeBimbinganInstansi.tanggal"
                        :feedback="edit_instansi_validation.tanggal.feedback"
                        :invalid="edit_instansi_validation.tanggal.invalid" />
                </CCol>
            </CRow>
            <CRow>
                <CCol>
                    <CFormTextarea id="data_bimbingan_instansi_edit" placeholder="Bimbingan"
                    label="Bimbingan"
                    v-model="activeBimbinganInstansi.data_bimbingan"
                    :feedback="edit_instansi_validation.data_bimbingan.feedback"
                    :invalid="edit_instansi_validation.data_bimbingan.invalid" />
                </CCol>
            </CRow>
        </CModalBody>
        <CModalFooter>
            <CButton color="success" @click="putBimbinganInstansi">Update</CButton>
            <CButton color="secondary" @click="closeModal">
                Close
            </CButton>
        </CModalFooter>
    </CModal>
    <CModal size="lg" backdrop="static" :visible="showTambahInstansiModal" @close="closeModal">
        <CModalHeader>
            <CModalTitle>Tambah Bimbingan Instansi</CModalTitle>
        </CModalHeader>
        <CModalBody>
            <CRow>
                <CCol>
                    <CFormInput type="date" id="tanggal_instansi" placeholder="Tanggal"
                        label="Tanggal"
                        v-model="form_instansi.tanggal"
                        :feedback="form_instansi_validation.tanggal.feedback"
                        :invalid="form_instansi_validation.tanggal.invalid" />
                </CCol>
            </CRow>
            <CRow>
                <CCol>
                    <CFormTextarea id="data_bimbingan_instansi" placeholder="Bimbingan"
                    label="Bimbingan"
                    v-model="form_instansi.data_bimbingan"
                    :feedback="form_instansi_validation.data_bimbingan.feedback"
                    :invalid="form_instansi_validation.data_bimbingan.invalid" />
                </CCol>
            </CRow>
        </CModalBody>
        <CModalFooter>
            <CButton color="success" @click="postBimbinganInstansi">Tambah</CButton>
            <CButton color="secondary" @click="closeModal">
                Close
            </CButton>
        </CModalFooter>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showTambahInstansiModal: false,
        showEditInstansiModal: false,
    ...
    methods: {
        ...
        bimbinganInstansiSearch(offset, limit, order, sort) {
            ...
            //calculate page based on offset and limit
            let page = offset / limit + 1;
            let url = `${window.location.origin}/api/kmm/magang/bimbingan/instansi?kueri=${this.searchBimbinganInstansi}&page=${page}&limit=${limit}&order=${order}&sort=${sort}`;
            axios.get(url)
            .then((response) => {
        ...
        postBimbinganInstansi() {
            this.$store.dispatch('showLoadingAlert');
            axios.post(`${window.location.origin}/api/kmm/magang/bimbingan/instansi`, this.form_instansi)
            .then((response) => {
                this.$store.dispatch('showSuccessAlert', 'Item Updated successfully!');
                this.getBimbinganInstansi();
                this.showTambahInstansiModal = false;
            })
            .catch((error) => {
                if (error.response.status === 422) {
                    this.form_instansi_validation = {
                        tanggal: {
                            invalid: !!error.response.data.errors.tanggal,
                            feedback: error.response.data.errors.tanggal.join(' & ')
                        },
                    ...
                    this.$store.dispatch('showErrorAlert', {
                        title: 'Gagal menambahkan bimbingan instansi!',
                        message: error.response.data.message
                    });
                } else {
                    console.log(error);
        ...
        openEditInstansiModal(item) {
            this.activeBimbinganInstansi = item;
            this.showEditInstansiModal = true;
        },
        putBimbinganDosen() {
            this.$store.dispatch('showLoadingAlert');
        ...
        putBimbinganInstansi() {
            this.$store.dispatch('showLoadingAlert');
            axios.put(`${window.location.origin}/api/kmm/magang/bimbingan/instansi/${this.activeBimbinganInstansi.id_bimbingan_instansi}`, this.activeBimbinganInstansi)
            .then((response) => {
                this.$store.dispatch('showSuccessAlert', 'Item Updated successfully!');
                this.getBimbinganInstansi();
                this.showEditInstansiModal = false;
            })
            .catch((error) => {
                if (error.response.status === 422) {
                    ...
                    this.$store.dispatch('showErrorAlert', {
                        title: 'Gagal mengubah bimbingan instansi!',
                        message: error.response.data.message
                    });
                } else {
                    console.log(error);
        ...
        deleteBimbinganInstansi(item) {
            this.$store.dispatch('showLoadingAlert');
            axios.delete(`${window.location.origin}/api/kmm/magang/bimbingan/instansi/${item.id_bimbingan_instansi}`)
            .then((response) => {
                this.$store.dispatch('showSuccessAlert', 'Item Deleted successfully!');
                this.getBimbinganInstansi();
            })
        ...
        closeModal() {
            ...
            this.showEditDosenModal = false;
            this.showTambahInstansiModal = false;
            this.activeBimbinganInstansi = null;
            this.showEditInstansiModal = false;
        }
    }
  }
